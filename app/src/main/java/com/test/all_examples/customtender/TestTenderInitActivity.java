...
public class TestTenderInitActivity extends Activity {
    ...
    if (tenders != null) {
      ...
      for (Tender tender : tenders) {
        if (getString(R.string.tender_name).equals(tender.getLabel())) {
          if (tender.getLabelKey().equals(getPackageName())) {
            tenderExists = true;
          } else {
            deleteTenderType(tender);
          }
        }
      }
  ...
  private void deleteTenderType(final Tender tender) {
    new AsyncTask<Void, Void, Exception>() {
      @Override
      protected Exception doInBackground(Void... params) {
        try {
          tenderConnector.deleteTender(tender.getId());
