...
public class CustomerFacingStationExample extends Activity {
  ...
  private void setLayout(Configuration newConfig) {
    if (newConfig.orientation == Configuration.ORIENTATION_PORTRAIT) {
      ...
      getActionBar().hide();
      setContentView(R.layout.activity_customer_facing_station_customer);
    } else {
      findViewById(android.R.id.content).getRootView().setSystemUiVisibility(View.VISIBLE);
      getActionBar().show();
      setContentView(R.layout.activity_customer_facing_station);
    }
  }
}
